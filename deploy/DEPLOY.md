...
4. 正常停止旧进程；如果应用端口仍被占用，则停止占用进程并启动新版本。
...
部署脚本将 8080 视为 `yc-page` 独占端口。停止旧应用后如果端口仍被占用，脚本会先发送 `TERM`，等待 30 秒后仍未释放则发送 `KILL`，然后启动新版本。

如果占用进程被 systemd 等服务自动重新拉起，脚本会停止部署并显示监听信息。可用下面的命令检查：

```bash
ss -ltnp | grep ':8080'
```

从输出取得 PID 后查看完整命令和所属服务：
